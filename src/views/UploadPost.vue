    ...
    <div class="d-flex p-lg-5">
        <div class="btn-group btn-group-vertical justify-content-start d-flex flex-column" style="width: 13vw; margin-right: 50px;" role="group" aria-label="Basic radio toggle button group">
            <div class="w-100" style="margin-bottom: 20px;">
                <input @click="clearForm()" type="radio" class="btn-check" name="btnradio" id="clear" autocomplete="off" :value="0" checked>
            ...
            <div v-for="(post, index) in posts" :key="post.id" class="w-100">
                <input @click="changeForm(post.id); choosenCreatedPostIndex = index; choosenCreatedPostId = post.id" type="radio" class="btn-check" name="btnradio" :id="'btnradio'+post.id" autocomplete="off" :value="post.id">
                <label class="btn btn-outline-dark w-100" :for="'btnradio'+post.id">{{ post.title }}</label>
            </div>

            <h5 class="text-center" style="margin-top: 20px;" v-if="postsReviewing.length >0">Các bài viết đang chờ phê duyệt </h5>
            <div v-for="(post, index) in postsReviewing" :key="post.id" class="w-100">
                <input @click="changeForm(post.id); choosenCreatedPostIndex = index; choosenCreatedPostId = post.id" type="radio" class="btn-check" name="btnradio" :id="'btnradio'+post.id" autocomplete="off" :value="post.id">
                <label class="btn btn-outline-dark w-100" :for="'btnradio'+post.id">{{ post.title }}</label>
            </div>
        </div>
...
\                <option v-for="cate in categories" :key="cate.id" :value="cate.id">
            ...
            <div> 
                ...
                <div class="btn-group" style="margin-bottom: 30px; flex-wrap: wrap;" role="group" aria-label="Basic checkbox toggle button group">
                    <div v-for="(tag, index) in filterTagsByCategoryId()" :key="tag.id">
                        <input v-model="trackingTags[index]" type="checkbox" class="btn-check" :id="'btncheck' + tag.id" autocomplete="off">
            ...
            <div class="mb-3 text-center d-flex flex-column justify-content-center" style="margin: 50px 0 50px 0">
                <img v-if="editPostForm.imageCover[0] == 'f'" style="margin-bottom: 50px;" class="img-thumbnail" width="500px" :src="'http://localhost:8080' + editPostForm.imageCover.replace('files', '')" alt="">
